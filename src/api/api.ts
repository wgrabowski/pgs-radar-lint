...
export async function getRadars(): Promise<PGSRadarInfo[]> {
	return Promise.all([getFeatures(), getAllRadars()]).then(
		([features, radars]) => {
...
async function getAllRadars(): Promise<PGSRadarInfo[]> {
	return fetch(API_ENDPOINTS.RADARS()).then((r) => r.json()) as Promise<
		PGSRadarInfo[]
	>;
}

async function getFeatures(): Promise<RadarFeatures[]> {
	return fetch(API_ENDPOINTS.FEATURES()).then((r) => r.json()) as Promise<
		RadarFeatures[]
	>;
}
